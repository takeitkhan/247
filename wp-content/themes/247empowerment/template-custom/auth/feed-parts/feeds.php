...
if ($query->have_posts()):
    while ($query->have_posts()): $query->the_post();
        ...
        if (is_user_logged_in()) {
            clean_user_reactions($post_id, $user_id);
        }
...
?>
        <div class="bg-white mb-3 post-card">
            ...
            $user_reactions = is_user_logged_in() ? get_user_reactions($post_id, $user_id) : [];
                ...
                <div class="border-top pt-2 px-2">
                    ...
                    <div class="mt-2 pt-2 border-top">
                        <div class="d-flex gap-2 align-items-flex-start">
                            <img src="<?php echo esc_url(get_user_meta($user_id, 'profile_photo', true) ?: get_template_directory_uri() . '/assets/img/loggedin_images/banner.jpg'); ?>" alt="You" class="rounded-circle" width="32" height="32" style="object-fit: cover;">
                            <div class="flex-grow-1">
                                <input type="text" class="form-control form-control-sm comment-input" data-post-id="<?php echo $post_id; ?>" placeholder="Write a comment... (Press Enter to post)">
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
